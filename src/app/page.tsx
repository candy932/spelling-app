...
export default function Home() {
  ...
  /**
   * 【修复核心】用 onChange + 纯 setState，不再直接操作 DOM
   *
   * 之前的问题：
   *   1. onInput 里直接写 input.value = clean，绕过 React
   *   2. defaultValue + 条件性 value 导致非受控?受控切换
   *   3. setState 使用闭包中的 currentWordIndex 可能是旧值
   *
   * 修复：
   *   1. 改用 onChange，只通过 setState 更新
   *   2. 去掉 defaultValue，始终用 value={blankPos.userAnswer} 受控
   *   3. 使用函数式 setState (prev => ...) 避免闭包旧值
   */
  ...
  const handleAddWord = () => {
    ...
    try {
      ...
      alert('添加成功！'); addInputRef.current?.focus()
  ...
  const saveEdit = () => {
    ...
    saveWords(newWords); setEditingWord(null); setEditEnglish(''); setEditChinese(''); alert('修改成功！')
  }
  ...
  /**
   * 【修复】checkAnswer 使用函数式 setState
   *
   * 之前：直接基于闭包中的 practiceWords 构建 npw
   *       → 如果 handleBlankInput 的 setState 还没被处理，practiceWords 是旧值
   *       → 基于旧值构建的新数组会丢失最后一次输入
   *
   * 修复：用 setPracticeWords(prev => ...) 确保基于最新 state
   */
  const checkAnswer = useCallback(() => {
    ...
    // 计算正确数也需要基于最新 state，用 ref 或者在 setState 回调中处理
    // 这里用 setTimeout 确保在 setState 完成后读取
    setPracticeWords(prev => {
      const cp = prev[currentWordIndex]
      if (cp && cp.isCorrect) {
  ...
  /**
   * 【修复核心】渲染空格单词 —— 纯受控 input
   *
   * 修改点：
   *   1. 去掉 defaultValue=""
   *   2. value 始终绑定 blankPos.userAnswer（来自 state）
   *   3. 用 onChange 替代 onInput
   *   4. 不再直接操作 input.value
   */
    ...
    return (
      <div key={`bw-${currentWordIndex}`} style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '6px', margin: '24px 0' }}>
        {letters.map((char, index) => {
            ...
            return (
              <div key={`b-${currentWordIndex}-${bi}`} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                <input
                  ...
                  // 【修复】始终使用 value 受控，值来自 state，不再用 defaultValue
          ...
          return (
            <div key={`c-${index}`} style={{ width: '40px', height: '52px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '22px', fontWeight: 'bold', color: '#475569', background: 'linear-gradient(to bottom, #f8fafc, #f1f5f9)', borderRadius: '12px', border: '2px solid #e2e8f0' }}>
              {char}
            </div>
          )
        })}
      ...
      <div style={{ minHeight: '100vh', background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
        <div style={{ width: '100%', maxWidth: '400px', background: 'white', borderRadius: '20px', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '24px' }}>
            <h2 style={{ fontSize: '20px', fontWeight: 'bold', marginBottom: '20px', textAlign: 'center' }}>?? 编辑单词</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div><label style={{ fontSize: '14px', color: '#64748b', marginBottom: '4px', display: 'block' }}>英文单词</label><input type="text" value={editEnglish} onChange={(e) => setEditEnglish(e.target.value)} style={{ width: '100%', padding: '12px', border: '2px solid #e2e8f0', borderRadius: '12px', fontSize: '16px', boxSizing: 'border-box' }} /></div>
              <div><label style={{ fontSize: '14px', color: '#64748b', marginBottom: '4px', display: 'block' }}>中文释义</label><input type="text" value={editChinese} onChange={(e) => setEditChinese(e.target.value)} style={{ width: '100%', padding: '12px', border: '2px solid #e2e8f0', borderRadius: '12px', fontSize: '16px', boxSizing: 'border-box' }} /></div>
  ...
  if (isPracticeMode && practiceWords.length > 0) {
    ...
    return (
      <div style={{ minHeight: '100vh', background: 'linear-gradient(to bottom right, #e0f2fe, #f5f3ff, #f3e8ff)', display: 'flex', flexDirection: 'column' }}>
        ...
        <main style={{ flex: 1, display: 'flex', flexDirection: 'column', maxWidth: '400px', margin: '0 auto', padding: '20px 16px', width: '100%' }}>
          <div style={{ flex: 1, background: 'white', borderRadius: '20px', boxShadow: '0 20px 40px -12px rgba(0, 0, 0, 0.2)', overflow: 'hidden' }}>
            ...
            <div style={{ textAlign: 'center', padding: '20px', background: 'linear-gradient(to bottom, #f8fafc, white)' }}>
              <p style={{ fontSize: '14px', color: '#64748b', marginBottom: '8px' }}>?? 请填写缺少的字母</p>
            ...
            <div style={{ padding: '16px' }}>
              ...
              {showAnswer && (
                <div style={{ textAlign: 'center', padding: '16px', borderRadius: '16px', marginBottom: '16px', background: cp.isCorrect ? '#d1fae5' : '#ffe4e6', border: `2px solid ${cp.isCorrect ? '#34d399' : '#fb7185'}` }}>
                  {cp.isCorrect ? <span style={{ fontWeight: 'bold', fontSize: '18px', color: '#047857' }}>? 正确！太棒了！</span> : <div><span style={{ fontWeight: 'bold', color: '#be123c' }}>? 再接再厉！</span><div style={{ marginTop: '8px', fontSize: '14px' }}>正确答案：<strong>{cp.word.english}</strong></div></div>}
                </div>
              )}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {!showAnswer ? (
        ...
        <div style={{ maxWidth: '400px', margin: '0 auto', padding: '0 16px 20px', width: '100%', textAlign: 'center', fontSize: '12px', color: '#94a3b8' }}>?? 点击格子 · 输入字母 · 自动跳格 · 退格删除</div>
      </div>
    )
  }
      ...
      <header style={{ position: 'sticky', top: 0, zIndex: 10, background: 'rgba(255, 255, 255, 0.9)', backdropFilter: 'blur(12px)', borderBottom: '1px solid #ede9fe' }}>
        <div style={{ maxWidth: '400px', margin: '0 auto', padding: '16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <h1 style={{ fontSize: '20px', fontWeight: 'bold', color: '#7c3aed' }}>单词拼写</h1>
          </div>
      ...
      <main style={{ maxWidth: '400px', margin: '0 auto', padding: '24px 16px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <div style={{ background: 'white', borderRadius: '20px', boxShadow: '0 20px 40px -12px rgba(0, 0, 0, 0.2)', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '32px', textAlign: 'center' }}>
            ...
            <p style={{ color: '#64748b', marginBottom: '20px' }}>随机抽取单词进行拼写练习</p>
